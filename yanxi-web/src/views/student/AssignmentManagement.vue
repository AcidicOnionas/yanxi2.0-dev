  ...
  <div class="assignment-management">
    ...
    <div class="assignment-section">
      ...
      <div class="assignment-list">
        ...
        <div v-else class="assignment-groups">
          <div v-for="(group, className) in groupedAssignments" :key="className" class="class-group">
            <div class="class-header">
              <h3 class="class-name">
                <i class="fas fa-book"></i>
                {{ className }}
                <span class="assignment-count">({{ group.length }} 个作业)</span>
              </h3>
            </div>
...
const groupedAssignments = computed(() => {
  ...
  assignmentList.value.forEach(assignment => {
    const className = assignment.className || `班级 ${assignment.classId}`
    if (!groups[className]) {
      groups[className] = []
    }
...
.class-group .assignment-table {
  box-shadow: none;
  border-radius: 0;
  margin: 0;
}

.class-group .assignment-table table {
  margin: 0;
}
